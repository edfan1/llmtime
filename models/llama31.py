...
def llama31_model_string(model_size, chat):
    chat = "chat-" if chat else ""
    return f"meta-llama/Meta-Llama-3.1-{model_size.upper()}{chat}"

def get_tokenizer(model):
    name_parts = model.split("-")
    model_size = name_parts[0]
    chat = len(name_parts) > 1
    assert model_size in ["8B", "70B", "405B"]
...
def llama31_nll_fn(model, input_arr, target_arr, settings:SerializerSettings, transform, count_seps=True, temp=1, cache_model=True):
